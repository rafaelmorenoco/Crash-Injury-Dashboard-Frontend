...
{#if yoy_text_fatal[0].is_first_week}
  <span style="font-size: 1.4rem; font-weight: 700;">
    <Value data={yoy_text_fatal} column="current_year" fmt="####"/> Year-End
  </span>
{:else}
{/if}
...
<ul class="markdown">
  ...
  {#if yoy_text_major_injury[0].is_first_week}
    <li>
      ...
      />)
      compared to the same period in
      <Value data={yoy_text_major_injury} column="year_prior" fmt="####."/>
    </li>
  {:else}
    <li>
      As of
...
-- Count Major injuries in the current year (based on frozen date)
major_counts AS (
    SELECT
        SUM(c.COUNT) AS major_this_year
    FROM crashes.crashes c
    CROSS JOIN date_range dr
    WHERE c.SEVERITY = 'Major'
      AND EXTRACT(YEAR FROM c.REPORTDATE) =
...
params AS (
    SELECT
        ...
        -- inFirstWeek OR noMajorThisYear
        CASE
            WHEN (
                -- First week of data: Jan 1–9
                (extract(month FROM current_date) = 1 AND extract(day FROM current_date) <= 9)
                OR
                -- Fallback: no Major injuries yet this year
                (SELECT major_this_year FROM major_counts) = 0
            )
            THEN TRUE
            ELSE FALSE
        END AS is_first_week
